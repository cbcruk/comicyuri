/**
 * 리더의 이야기. `src/page/reader/story.test.ts`를 옮긴 것이다.
 *
 * 테스트 이름과 단언은 옮기기 전과 같다. 달라진 것은 페이지를 부르는 일이 사라진
 * 자리뿐이다 — `LoadSpread`와 `PreloadNeighbours`, `LoadThumbs`를 두고 주고받던 걸음은
 * 이제 atom의 일이라 여기 없고, 무엇이 한 화면인지는 `spreadPages`에 묻는다.
 */
...
import { DOUBLE_TAP_ZOOM, ORIGIN, ZOOM_MIN } from '../reader/gesture.ts'
import type { Point } from '../reader/gesture.ts'
import { NO_ROOM } from '../reader/scroll.ts'
import type { ScrollDevice } from '../reader/scroll.ts'
...
/** 책을 열고 첫 스프레드가 걸린 것까지 본다. 모든 테스트가 여기서 시작한다. */
const opened = (page: number, ratios: ReadonlyArray<Option.Option<number>> = UNMEASURED) => [
  ...
  model((model) => {
    ...
    expect(spreadPages(model)).toStrictEqual([page])
  }),
]
  ...
  test('a book that opens shows its first spread without claiming that as progress', () => {
    ...
    story(
      ...
      model((model) => {
        ...
        expect(spreadPages(model)).toStrictEqual([0])
      }),
    )
  })
...
describe('layout', () => {
  ...
  test('a wide page is read on its own and the pairs after it stay in step', () => {
    story(
      ...
      model((model) => {
        expect(model.page).toBe(1)
        expect(spreadPages(model)).toStrictEqual([1, 2])
      }),
      message(Message.ClickedNext()),
      model((model) => {
        expect(model.page).toBe(3)
        expect(spreadPages(model)).toStrictEqual([3])
      }),
      // 넓은 페이지 하나가 그 뒤를 한 장씩 밀어내지 않는다.
      message(Message.ClickedNext()),
      model((model) => {
        expect(model.page).toBe(4)
        expect(spreadPages(model)).toStrictEqual([4, 5])
      }),
    )
  })
  ...
  test('the page before a wide one is read alone rather than paired across it', () => {
    story(
      ...
      model((model) => {
        expect(model.page).toBe(1)
        expect(spreadPages(model)).toStrictEqual([1])
      }),
      message(Message.ClickedNext()),
      model((model) => {
        expect(model.page).toBe(2)
        expect(spreadPages(model)).toStrictEqual([2])
      }),
    )
  })
  ...
  test('flipping binds a wide page back to its neighbour', () => {
    story(
      ...
      model((model) => {
        expect(model.page).toBe(3)
        expect(spreadPages(model)).toStrictEqual([3, 4])
      }),
    )
  })
  ...
  test('a setting picked in the panel lays the book out again at once', () => {
    story(
      ...
      model((model) => {
        expect(model.page).toBe(0)
        expect(spreadPages(model)).toStrictEqual([0, 1])
      }),
    )
  })
