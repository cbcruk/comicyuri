/**
 * Message 하나를 리더에 접어 넣는 자리. 순수한 함수라 아무것도 실행하지 않는다.
 *
 * 돌려주는 것은 다음 Model과, 바깥에 부탁할 Command와, 위로 올려 보낼 OutMessage뿐이다.
 * 페이지를 부르고 놓아 주는 일은 `src/atoms/pages.ts`의 atom이 맡으므로 여기 없다.
 */

import { Array, Option, Order } from 'effect'

import { Reading } from '../domain/index.ts'
import { bookmarkFrom } from '../reader/bookmark.ts'
import { ORIGIN } from '../reader/gesture.ts'
import { rotatedRight } from '../reader/rotation.ts'
...
export const update = (model: Model, message: Message): UpdateReturn =>
  Message.$match(message, {
    ...
    ClickedPrevious: (): UpdateReturn => step(model, -1),
    ClickedNext: (): UpdateReturn => step(model, 1),
    ClickedFirst: (): UpdateReturn => goToPage(model, 0),

    ClickedLast: (): UpdateReturn =>
      OpenState.$match(model.openState, {
        Opening: (): UpdateReturn => ({ model }),
        Failed: (): UpdateReturn => ({ model }),
    ...
    ClickedExit: (): UpdateReturn => ({ model, outMessage: OutMessage.RequestedExit() }),

    ClickedToggleDirection: (): UpdateReturn =>
      withSettings(
        model,
        evo(model.settings, {
    ...
    ClickedSkip: ({ pages }): UpdateReturn => skip(model, pages),

    /**
     * 적어 넣은 번호로 간다. 책 밖의 번호나 숫자가 아닌 것은 아무 일도 일으키지
     * 않는다 — 잘못 적은 것을 되돌릴 자리가 입력란 자신이기 때문이다.
     */
    SubmittedGoToPage: ({ text }): UpdateReturn =>
      OpenState.$match(model.openState, {
        Opening: (): UpdateReturn => ({ model }),
        Failed: (): UpdateReturn => ({ model }),
     ...
     * 따라간다.
     */
    ToggledRememberBookSettings: ({ isChecked }): UpdateReturn =>
      withSettings(
        model,
        isChecked
    ...
    SelectedAtBookEnd: ({ atBookEnd }): UpdateReturn =>
    ...
    ClickedDismissResume: (): UpdateReturn => ({
      model: evo(model, { maybeResumePage: () => Option.none<number>() }),
    }),

    ClickedToggleBookmark: (): UpdateReturn => {
    ...
    // 그것으로 맞다.
    ClickedToggleFullscreen: (): UpdateReturn => ({
      model,
      commands: [ToggleFullscreen({ wantFullscreen: !model.isFullscreen })],
    }),

    CompletedToggleFullscreen: (): UpdateReturn => ({ model }),

    ChangedFullscreen: ({ isFullscreen }): UpdateReturn => ({
     ...
     */
    ClickedStepBookmark: ({ step: by }): UpdateReturn =>
      Option.match(bookmarkFrom(model.bookmarks, model.page, by), {
        onNone: (): UpdateReturn => ({ model }),
        onSome: (page): UpdateReturn => goToPage(model, page),
      }),

    SelectedThumb: ({ page }): UpdateReturn => selectedThumb(model, page),

    PressedPointer: ({ pointerId, at }): UpdateReturn => pressedPointer(model, pointerId, at),
    MovedPointer: ({ pointerId, at }): UpdateReturn => movedPointer(model, pointerId, at),
    ReleasedPointer: ({ pointerId, at, timeStamp, viewportWidth }): UpdateReturn =>
      releasedPointer(model, pointerId, at, timeStamp, viewportWidth),
    CancelledPointer: (): UpdateReturn => droppedGesture(model),
    AbandonedPointer: (): UpdateReturn => droppedGesture(model),

    ScrolledToZoom: ({ delta, at }): UpdateReturn => ({
    ...
    ScrolledStage: ({ delta, room, device }): UpdateReturn => {
      ...
      return Option.match(turnFromEdge(delta, room), {
        onNone: (): UpdateReturn => ({ model }),
        onSome: (by): UpdateReturn => step(model, by),
      })
    },

    ClickedZoomIn: (): UpdateReturn => ({
      model: zoomedTo(model, model.zoom * 1.25, ORIGIN),
    }),

    ClickedZoomOut: (): UpdateReturn => ({
      model: zoomedTo(model, model.zoom / 1.25, ORIGIN),
    }),

    ClickedToggleChrome: (): UpdateReturn => ({
      model: evo(model, { isChromeVisible: (isVisible) => !isVisible }),
    }),

    PressedKey: ({ key, withShift }): UpdateReturn =>
      Option.match(messageForKey(model, key, withShift), {
        onNone: (): UpdateReturn => ({ model }),
        onSome: (keyMessage): UpdateReturn => update(model, keyMessage),
      }),
  })
