...
export interface Page {
  ...
  /** 캐시해 둔 object URL을 놓아 메모리를 돌려준다. 다시 `load()`하면 새로 만든다. */
  unload(): void
  /**
   * 책을 닫는다. 캐시해 둔 URL을 놓고, 그 뒤에 끝나는 `load()`는 URL을 캐시하지 않고
   * 곧바로 놓는다.
   *
   * `unload()`와 따로 두는 이유는 닫힌 페이지에는 URL을 놓을 다음 기회가 없기 때문이다.
   * 압축을 푸는 도중에 책을 떠나면 푸는 일은 닫힌 뒤에 끝나고, 그때 캐시한 URL은 페이지
   * 객체와 함께 버려진다. 미리 읽기가 부르는 `unload()`에 같은 규칙을 걸면 안 된다 — 늦게
   * 끝난 이전 미리 읽기가 지금 불러오는 페이지를 놓았을 때, 그 페이지의 URL까지 놓아 버린다.
   */
  release(): void
  /**
