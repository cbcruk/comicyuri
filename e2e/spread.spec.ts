...
import { control, readBook, readMenuItem, stage, use } from './fixture/app.ts'
...
/**
 * 보기 항목이 곁글에 적고 있는 지금 값. 예전에는 툴바 버튼의 글자였다.
 */
const expectView = (page: Page, value: string) =>
  readMenuItem(page, 'view', async (item) => {
...
test('R-227 · 한 장 모드에는 뒤집을 묶기가 없다', async ({ page }) => {
  await readBook(page, withWideFourthPage)
  await readMenuItem(page, 'binding', async (item) => {
